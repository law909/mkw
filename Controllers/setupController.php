...
//        $view->setVar(\mkw\consts::Tulajcrc, ($p ? $p->getErtek() : ''));
        ...
        $p = $repo->find(\mkw\consts::IdopontfoglalasSablonKoszono);
        $ifsablon = new emailtemplateController();
        $view->setVar('idopontfoglalaskoszonoemailsablonlist', $ifsablon->getSelectList(($p ? $p->getErtek() : 0)));

        $p = $repo->find(\mkw\consts::IdopontfoglalasSablonEmlekezteto);
        $ifsablon = new emailtemplateController();
...
//        $meretcs = new termekvaltozatadattipusController();
    ...
    private const CONDITIONAL_BLOCKS = [
        ...
        'jogafulvan' => [
            ...
            \mkw\consts::AdategyeztetoSablon,
        ],
        'teljesitmenyvan' => [
    ...
    public function save()
    {
        ...
        $szkt = \mkw\store::getEm()->getRepository(Termek::class)->find($this->params->getIntRequestParam('utanvetktgtermek', 0));
        if ($szkt) {
            $this->setObj(\mkw\consts::UtanvetKtgTermek, $szkt->getId());
        } else {
            $this->setObj(\mkw\consts::UtanvetKtgTermek, '');
        }

        $szkt = \mkw\store::getEm()->getRepository(Termek::class)->find($this->params->getIntRequestParam('koltsegtermek', 0));
        if ($szkt) {
        ...
        $levelsablon = \mkw\store::getEm()->getRepository(Emailtemplate::class)->find(
            $this->params->getIntRequestParam(\mkw\consts::IdopontfoglalasSablonKoszono, 0)
        );
        if ($levelsablon) {
            $this->setObj(\mkw\consts::IdopontfoglalasSablonKoszono, $levelsablon->getId());
        } else {
            $this->setObj(\mkw\consts::IdopontfoglalasSablonKoszono, '');
        }

        $levelsablon = \mkw\store::getEm()->getRepository(Emailtemplate::class)->find(
            $this->params->getIntRequestParam(\mkw\consts::IdopontfoglalasSablonEmlekezteto, 0)
        );
        if ($levelsablon) {
        ...
        $levelsablon = \mkw\store::getEm()->getRepository(Emailtemplate::class)->find(
            $this->params->getIntRequestParam(\mkw\consts::AdategyeztetoSablon, 0)
        );
        if ($levelsablon) {
            $this->setObj(\mkw\consts::AdategyeztetoSablon, $levelsablon->getId());
        } else {

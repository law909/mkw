...
/**
 * Galád termékimport a SUOMY-formátumú (megtisztított) XLSX-ből.
 *
 * Oszlopok: A=Változat (0/1 – változatos-e a termék), B=Közös cikkszám (a termék
 * cikkszáma ÉS a változatokat összekötő csoportkulcs), C=Termék neve, D=Szín, E=Méret,
 * F=CIKKSZÁM (a változat cikkszáma – önmagában egyedi), G=MEGNEVEZÉS (nem használt),
 * H=KISKER ÁR (bruttó), I=VONALKÓD, J=tipus (kategória / termékfa).
 *
 * Az azonos közös cikkszámú (B oszlop) sorok egy termékhez tartoznak; ha A=1, a sorok
 * szín/méret változatok.
 */
class galadSuomyImportController extends \mkwhelpers\Controller
{
     ...
     *
     * - Az ár nélküli (üres H oszlop) sorokat is importálja, csak árat nem állít be hozzájuk.
     * - A közös cikkszám (B oszlop) köti össze egy termék sorait.
     * - Ha a változat (A oszlop) == 1, a termék sorai szín/méret változatok.
     *   A változat cikkszáma az F oszlop (önmagában egyedi).
     * - A kategória (J oszlop) szövegével hasonló nevű termékfa csomópontot keres, ahhoz
     *   kapcsolja a terméket (termekfa1).
     * - A kisker árat (H) a "Kisker.ár" ársávba tölti (létrehozza, ha még nincs).
     */
    public function import()
    {
        ...
        $filenev = \mkw\store::moveUploadedFile('toimport', 'suomyimport');
        if (!$filenev) {
        ...
        // gyártó partner ("SUOMY"), ha még nincs (szállító = 1)
        $gyarto = $this->getOrCreatePartner('SUOMY MOTOSPORT Srl');
        ...
        for ($row = $dbtol; $row <= $hardMax; ++$row) {
            ...
            $nev = trim((string)$sheet->getCell('C' . $row)->getValue());
            $szin = trim((string)$sheet->getCell('D' . $row)->getValue());
            $meret = trim((string)$sheet->getCell('E' . $row)->getValue());
            $cikkszam = trim((string)$sheet->getCell('F' . $row)->getValue());
            $kiskerAr = $sheet->getCell('H' . $row)->getValue();
            $vonalkod = trim((string)$sheet->getCell('I' . $row)->getValue());
            $kategoria = trim((string)$sheet->getCell('J' . $row)->getValue());

            $hUres = ($kiskerAr === null || trim((string)$kiskerAr) === '');

            // teljesen üres sor: az adatok végét egy hosszabb üres sorozat jelzi
            if ($valtozat === '' && $kozoscikkszam === '' && $nev === '' && $szin === ''
                && $meret === '' && $cikkszam === '' && $vonalkod === '' && $kategoria === ''
                && $hUres) {
            ...
            // a termék neve (C oszlop) nélküli sorokat nem importáljuk
            if ($nev === '') {
                continue;
            }
            ...
            $groups['k:' . $kozoscikkszam][] = [
                'valtozatos' => $valtozat,
                'kozoscikkszam' => $kozoscikkszam,
                'nev' => $nev,
                'szin' => $szin,
                'meret' => $meret,
                'cikkszam' => $cikkszam,
        ...
        foreach ($groups as $group) {
            $mindenTermekKulcs[] = $group[0]['kozoscikkszam'];
    ...
    /**
     * Termékfa csomópont keresése név (J/kategória oszlop) alapján: pontos egyezés,

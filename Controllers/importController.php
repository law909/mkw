...
class importController extends \mkwhelpers\Controller {
    ...
    public function deltonImport() {
        ...
        $ch = \curl_init('http://www.holdpeak.hu/csv.php');
        ...
        if ($fh) {
            ...
            while (($termekdb < $dbtol) && ($data = stream_get_line($fh, 0,"\n"))) {
                $termekdb++;
            }
            while ((($dbig && ($termekdb < $dbig)) || (!$dbig)) && ($data = stream_get_line($fh, 0, "\n"))) {
                $data = explode(chr(99), $data);
                $termekdb++;

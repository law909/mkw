                    ...
                    foreach ($valtozatok as $valt) {
                        if ($valt->getXElerheto() && $valt->getXLathato()) {
                            if ($valt->getAdatTipus1Id() == \mkw\store::getParameter(\mkw\consts::ValtozatTipusSzin)) {
                                ...
                                \mkw\store::writelog($valt->getErtek1() . ' ' . $valt->getErtek2());
                                \mkw\store::writelog($valt->getKeszlet());
                                \mkw\store::writelog($valt->getFoglaltMennyiseg());
                                \mkw\store::writelog($valt->calcMinboltikeszlet());

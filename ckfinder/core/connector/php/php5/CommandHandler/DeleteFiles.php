...
class CKFinder_Connector_CommandHandler_DeleteFiles extends CKFinder_Connector_CommandHandler_XmlCommandHandlerBase
{
  ...
  private function appendErrorNode($oErrorsNode, $errorCode, $name, $type, $path)
  {
	...
	//+law
	private function deleteThumbs($filePath) {
		$res=true;
		foreach($GLOBALS['config']['Plugin_Serverresize']['sizes'] as $k=>$s) {
			$source=CKFinder_Connector_Utils_FileSystem::getFileNameWithoutExtension($filePath) .
				"_".$k.".".CKFinder_Connector_Utils_FileSystem::getExtension($filePath);
			if (!@unlink($source)) {
				$res=false;
				continue;
			}

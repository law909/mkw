...
class CKFinder_Connector_CommandHandler_CopyFiles extends CKFinder_Connector_CommandHandler_XmlCommandHandlerBase
{
                ...
                // check if file exists if we don't force overwriting
                    ...
                    if (strpos($options, "autorename") !== false) {
                        ...
                        if (!@copy($sourceFilePath, $destinationFilePath)) {
                            ...
                            $copied++;
							//+law
							if (!$this->copyThumbs($name, $sourceFilePath, $sServerDir)) {
								$errorCode = CKFINDER_CONNECTOR_ERROR_ACCESS_DENIED;
								$this->appendErrorNode($oErrorsNode, $errorCode, $name, $type, $path);
							}
							//-law
	...
	//+law
	private function copyThumbs($name,$sourceFilePath,$sServerDir) {
		$res=true;
		foreach($GLOBALS['config']['Plugin_Serverresize']['sizes'] as $k=>$s) {
			$dest=$sServerDir.CKFinder_Connector_Utils_FileSystem::getFileNameWithoutExtension($name) .
				"_".$k.".".CKFinder_Connector_Utils_FileSystem::getExtension($name);

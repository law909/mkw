...
class CKFinder_Connector_CommandHandler_MoveFiles extends CKFinder_Connector_CommandHandler_XmlCommandHandlerBase
{
                    ...
                    if (strpos($options, "overwrite") !== false) {
                        if (!@unlink($destinationFilePath)) {
                            ...
                            if (!@rename($sourceFilePath, $destinationFilePath)) {
								...
								//+law
								if (!$this->renameThumbs($sourceFilePath, $destinationFilePath)) {
									$errorCode = CKFINDER_CONNECTOR_ERROR_ACCESS_DENIED;
									$this->appendErrorNode($oErrorsNode, $errorCode, $name, $type, $path);
									continue;
								}
	...
	//+law
	private function renameThumbs($sourceFilePath,$destinationFilePath) {
		$res=true;
		foreach($GLOBALS['config']['Plugin_Serverresize']['sizes'] as $k=>$s) {
			$dest=CKFinder_Connector_Utils_FileSystem::getFileNameWithoutExtension($destinationFilePath) .
				"_".$k.".".CKFinder_Connector_Utils_FileSystem::getExtension($destinationFilePath);
			$source=CKFinder_Connector_Utils_FileSystem::getFileNameWithoutExtension($sourceFilePath) .
				"_".$k.".".CKFinder_Connector_Utils_FileSystem::getExtension($sourceFilePath);

...
class TermekMenu2Repository extends \mkwhelpers\Repository
{
    ...
    public function __construct($em, \Doctrine\ORM\Mapping\ClassMetadata $class)
    {
        ...
        $this->setEntityname(TermekMenu2::class);
        $this->setOrders([
    ...
    public function regenerateKarKod()
    {
        ...
        $q = $this->_em->createNativeQuery('SELECT id,parent_id FROM termekmenu2 ORDER BY parent_id,id', $rsm);
        ...
        foreach ($this->fatomb as $key => $val) {
            if ($val['parent_id'] == $szuloid) {
                $q = $this->_em->createQuery(
                    'UPDATE Entities\TermekMenu2 x SET x.karkod=\'' . $szulokarkod . sprintf('%05d', $val['id']) . '\' WHERE x.id=' . $val['id']
                );
                $q->Execute();
                $q = $this->_em->createQuery(
                    'UPDATE Entities\Termek x SET x.termekmenu2karkod=\'' . $szulokarkod . sprintf('%05d', $val['id']) . '\' WHERE x.termekmenu2=' . $val['id']
                );
                $q->Execute();
    ...
    public function getForMenu($menunum, $webshopnum = null)
    {
        ...
        $q = $this->_em->createQuery('SELECT f FROM Entities\TermekMenu2 f WHERE f.menu' . $menunum . 'lathato=1 ' . $webshopfilter . ' ORDER BY f.sorrend');
        $res = $q->getResult();
        $ret = [];
        /** @var TermekMenu2 $r */
        foreach ($res as $r) {
    ...
    public function getForFilter($webshopnum = null)
    {
        ...
        $q = $this->_em->createQuery('SELECT f FROM Entities\TermekMenu2 f WHERE ' . $webshopfilter . ' ORDER BY f.sorrend,f.nev');
        $res = $q->getResult();
        $ret = [];
        /** @var TermekMenu2 $r */
        foreach ($res as $r) {
    ...
    public function getForSitemapXml()
    {
        ...
        $q = $this->_em->createNativeQuery(
            ...
            . ' FROM termekmenu2 WHERE ((inaktiv=0) OR (inaktiv IS NULL)) AND ((menu1lathato=1) OR (menu2lathato=1) OR (menu3lathato=1) OR (menu4lathato=1))'
            . ' ORDER BY id',
            $rsm
        );
        return $q->getScalarResult();

...
class PenztarbizonylatfejRepository extends \mkwhelpers\Repository
{
    ...
    public function __construct($em, \Doctrine\ORM\Mapping\ClassMetadata $class)
    {
        ...
        $this->setOrders([
            ...
            '7' => ['caption' => 'irány és biz.szám szerint csökkenő', 'order' => ['_xx.irany' => 'DESC', '_xx.id' => 'DESC']],
            '8' => ['caption' => 'irány és biz.szám szerint növekvő', 'order' => ['_xx.irany' => 'DESC', '_xx.id' => 'ASC']]
        ]);
    }
    ...
    {
        $q = $this->_em->createQuery(
            'SELECT SUM(_xx.irany * _xx.brutto)'
            . ' FROM Entities\Penztarbizonylatfej _xx'
            . $this->getFilterString($filter)
        );
        $q->setParameters($this->getQueryParameters($filter));
    ...
    {
        $q = $this->_em->createQuery(
            'SELECT IDENTITY(_xx.penztar),p.nev,SUM(_xx.irany * _xx.brutto)'
            . ' FROM Entities\Penztarbizonylatfej _xx'

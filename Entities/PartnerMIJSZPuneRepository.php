...
class PartnerMIJSZPuneRepository extends \mkwhelpers\Repository {
    ...
    public function getWithJoins($filter, $order, $offset = 0, $elemcount = 0) {
        $q = $this->_em->createQuery('SELECT _xx, partner'
            . ' FROM Entities\PartnerMIJSZPune _xx'
    ...
    public function getLastByPartner($partner) {
        ...
        $kdv = $this->getWithJoins($filter, array('_xx.ev' => 'DESC', '_xx.honap' => 'DESC'));
        if ($kdv) {
            return $kdv[0];
        }
        return false;

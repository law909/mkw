...
class TermekcimkekatRepository extends \mkwhelpers\Repository
{
    ...
    public function __construct($em, \Doctrine\ORM\Mapping\ClassMetadata $class)
    {
        ...
        $this->setOrders([
            ...
            '3' => ['caption' => 'név szerint csökkenő', 'order' => ['_xx.nev' => 'DESC']],
        ]);
    }
    ...
    {
        $q = $this->_em->createQuery(
            'SELECT _xx.id,_xx.nev,_xx.slug,_xx.sorrend,c.id AS cid,c.nev AS cnev,c.sorrend AS csorrend'
            . ' FROM Entities\Termekcimkekat _xx'
            . ' LEFT JOIN _xx.cimkek c '
            . $this->getFilterString($filter)
            . $this->getOrderString($order)
    ...
    {
        // TODO SQLINJECTION
        $res = $this->getWithJoins($this->alias . '.id=' . $id, []);
        return $res[0];
    }
    ...
    public function getForTermekSzuro($termekid, $selectedids)
    {
        ...
        $order = ['_xx.sorrend' => 'asc', '_xx.nev' => 'asc', 'c.sorrend' => 'asc', 'c.nev' => 'asc'];
        $q = $this->_em->createQuery(
            'SELECT _xx,c,t'
            . ' FROM Entities\Termekcimkekat _xx'
            . ' LEFT JOIN _xx.cimkek c '

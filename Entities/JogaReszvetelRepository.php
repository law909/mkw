...
class JogaReszvetelRepository extends \mkwhelpers\Repository
{
    ...
    public function __construct($em, \Doctrine\ORM\Mapping\ClassMetadata $class)
    {
        ...
        $this->setOrders([
            '1' => [
                ...
                'order' => ['_xx.datum' => 'DESC', 'ta.nev' => 'ASC', 'pa.nev' => 'ASC']
            ],
            '2' => [
                'caption' => 'dátum és tanár szerint növekvő',
                'order' => ['_xx.datum' => 'ASC', 'ta.nev' => 'ASC', 'pa.nev' => 'ASC']
            ],
            '3' => [
                'caption' => 'tanár és dátum szerint növekvő',
                'order' => ['ta.nev' => 'ASC', '_xx.datum' => 'ASC', 'pa.nev' => 'ASC']
            ]
        ]);
    }
    ...
    public function getTanarOsszesito($filter, $honap = 1)
    {
        $res = [];
        $q = $this->_em->createQuery(
            'SELECT SUM(_xx.jutalek) AS jutalek,ta.nev,ta.id,ta.havilevonas*' . $honap . ' AS havilevonas,fm.nev AS fizmodnev,'
    ...
    {
        $q = $this->_em->createQuery(
            'SELECT COUNT(_xx) AS db,t.nev,t.id'
            . ' FROM Entities\JogaReszvetel _xx'
            . ' LEFT JOIN _xx.termek t'
            . $this->getFilterString($filter)
            . ' GROUP BY t.id'

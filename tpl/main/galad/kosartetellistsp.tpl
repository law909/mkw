...
<tr>
    ...
    <th>Termék</th>
    <th>
        <div class="textalignright">Listaár</div>
    </th>
    <th>
        <div class="textaligncenter">Kedvezmény %</div>
    </th>
    <th>
        <div class="textalignright">Egységár</div>
    </th>
    <th>
        <div class="textaligncenter">Menny.<i class="icon-question-sign cartheader-tooltipbtn hidden-phone js-tooltipbtn"
...
{foreach $tetellista as $tetel}
    ...
    <tr data-href="{$tetel.link}">
            ...
            <div class="textaligncenter">
                {if ($uzletkoto.loggedin)}
                    <form class="kosarformk" action="{$tetel.editlink}">
                        <div>{if ($tetel.noedit)}
                                {number_format($tetel.kedvezmeny|default:0, 0, ',', ' ')} %
                            {else}
        ...
        <td>
            <div id="egysegar_{$tetel.id}" class="textalignright">{number_format($tetel.bruttoegysar, 2, ',', ' ')} {$valutanem}</div>
        </td>
        ...
        <td>
            <div id="ertek_{$tetel.id}" class="textalignright">{number_format($tetel.brutto, 2, ',', ' ')} {$valutanem}</div>
        </td>
        <td class="textaligncenter">{if (!$tetel.noedit)}<a class="btn btn-default js-kosardelbtn" href="/kosar/del?id={$tetel.id}" rel="nofollow"><i
                    class="icon-remove-sign"></i>Töröl</a>{/if}</td>
    </tr>
{/foreach}
</tbody>
<tfoot>
<tr>
    ...
    <th>
        <div id="kosarsum" class="textalignright">{number_format($osszesen, 2, ',', ' ')} {$valutanem}</div>
    </th>
    <th></th>
</tr>
</tfoot>

						...
						<div id="tulajdonsagTab" class="tab-pane">
							...
							<table class="table table-striped table-condensed"><tbody>
								{foreach $termek.cimkelapon as $_cimke}
									<tr><td>{$_cimke.kategorianev}</td><td>{if ($_cimke.kiskepurl!='')}<img src="{$_cimke.kiskepurl}" alt="{$_cimke.caption}" title="{$_cimke.caption}"> {/if}{if (!$_cimke.dontshowcaption)}{$_cimke.caption}{/if}</td></tr>
								{/foreach}
							</tbody></table>
							</div>
						</div>

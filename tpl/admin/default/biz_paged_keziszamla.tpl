...
{block "datesrow"}
    <table class="fullwidth" cellspacing="0" cellpadding="0" border="0">
        <tr class="bold textaligncenter">
            <td width="13%">Kelt</td>
            <td width="13%">Teljesítés</td>
            <td width="13%">Fiz.határidő</td>
            <td width="16%">Fizetési mód</td>
            <td width="10%">Pénznem</td>
            <td width="17%">Eredeti biz.szám</td>
            <td width="18%">Biz. száma</td>
        </tr>
...
{block "itemrows"}
    <tr class="tetelsor">
        <td>{$teteldb + 1}</td>
        <td colspan="8" width="{$w.nevsor}" class="bold">{$tetel.cikkszam} {$tetel.termeknev} {foreach $tetel.valtozatok as $valtozat}{$valtozat.ertek}&nbsp;{/foreach}{if ($tetel.termekegyediazonosito|default)}({$tetel.termekegyediazonosito}) {/if}{if ($tetel.vtszszam|default)}({$tetel.vtszszam}){/if}</td>
    </tr>
    <tr class="tetelsor">

...
{block "inhead"}
    ...
    <script type="text/javascript" src="/js/admin/default/penztarbizonylattetel.js"></script>
{/block}
...
{block "kozep"}
    <div id="mattable-select" data-theme="{$theme}">
        ...
        <div id="mattable-filterwrapper">
            ...
            <div class="matt-hseparator"></div>
            <div>
                <label for="jogcimfilter">{at('Jogcím')}:</label>
                <select id="jogcimfilter" name="jogcimfilter">
                    <option value="0">{at('válasszon')}</option>
                    {foreach $jogcimlist as $jogcim}
                        <option value="{$jogcim.id}"{if ($jogcim.selected)} selected="selected"{/if}>{$jogcim.caption}</option>
                    {/foreach}
                </select>
            </div>
            <div class="matt-hseparator"></div>
        ...
        <table id="mattable-table">
            ...
            <tr>
                ...
                <th>{at('Partner')}</th>
                <th>{at('Kelt')}</th>
                <th>{at('Jogcím')} / {at('Hivatkozott bizonylat')}</th>
                <th>{at('Összeg')}</th>
            </tr>
            </thead>

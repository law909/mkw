...
<form id="mattkarb-form" method="post" action="/admin/termek/save" data-id="{$termek.id}">
    <div id="mattkarb-tabs">
        <ul>
            ...
            {if ($setup.arsavok)}
                <li><a href="#ArsavTab">{t('Ársávok')}</a></li>
            {/if}
            {if ($setup.multilang)}
                <li><a href="#TranslationTab">{t('Idegennyelvi adatok')}</a></li>
            {/if}
            <li><a href="#CimkeTab">{t('Címkék')}</a></li>
            <li><a href="#KepTab">{t('Képek')}</a></li>
            {if ($setup.receptura)}
                <li><a href="#RecepturaTab">{t('Receptúra')}</a></li>
            {/if}
        ...
        <div id="AltalanosTab" class="mattkarb-page" data-visible="visible">
            ...
            <table>
                ...
                <tr>
                    ...
                    <td><span id="TermekKategoria1" class="js-termekfabutton" data-text="{t('válasszon')}"
                              data-name="termekfa1"
                              data-value="{$termek.termekfa1}">{if ($termek.termekfa1nev)}{$termek.termekfa1nev}{else}{t('válasszon')}{/if}</span>
                    </td>
                    <td><span id="TermekKategoria2" class="js-termekfabutton" data-text="{t('válasszon')}"
                              data-name="termekfa2"
                              data-value="{$termek.termekfa2}">{if ($termek.termekfa2nev)}{$termek.termekfa2nev}{else}{t('válasszon')}{/if}</span>
                    </td>
                    <td><span id="TermekKategoria3" class="js-termekfabutton" data-text="{t('válasszon')}"
                              data-name="termekfa3"
                              data-value="{$termek.termekfa3}">{if ($termek.termekfa3nev)}{$termek.termekfa3nev}{else}{t('válasszon')}{/if}</span>
                    </td>
                </tr>
                </tbody>
            </table>
            <table>
                <tbody>
                <tr>
                    <td><label for="TermekcsoportEdit">{t('Termékcsoport')}:</label></td>
        ...
        {if ($setup.receptura)}
            <div id="RecepturaTab" class="mattkarb-page" data-visible="visible">
                ...
                {/foreach}
                <a class="js-receptnewbutton" href="#" title="{t('Új')}"><span
                        class="ui-icon ui-icon-circle-plus"></span></a>
            </div>
        {/if}
        {if ($setup.termekvaltozat)}
            <div id="ValtozatTab" class="mattkarb-page" data-visible="visible">
                ...
                <a class="js-valtozatdelallbutton" href="#" title="{t('Mind törlése')}"
                   data-termekid="{$termek.id}"><span class="ui-button-text">{t('Mind törlése')}</span></a>
                ...
                {/foreach}
                <a class="js-valtozatnewbutton" href="#" title="{t('Új')}" data-termekid="{$termek.id}"><span
                        class="ui-icon ui-icon-circle-plus"></span></a>
            </div>
        {/if}
        {if ($setup.kapcsolodotermekek)}
            <div id="KapcsolodoTab" class="mattkarb-page" data-visible="visible">
                ...
                {/foreach}
                <a class="js-kapcsolodonewbutton" href="#" title="{t('Új')}"><span
                        class="ui-icon ui-icon-circle-plus"></span></a>
            </div>
        {/if}
        <div id="WebTab" class="mattkarb-page"{if ($setup.editstyle=='dropdown')} data-visible="hidden"{/if}>
            <input id="LathatoCheck" name="lathato" type="checkbox"
                   {if ($termek.lathato)}checked="checked"{/if}>{t('Weboldalon látható')}
            <input id="NemkaphatoCheck" name="nemkaphato" type="checkbox"
                   {if ($termek.nemkaphato)}checked="checked"{/if}>{t('Nem kapható')}
            <input id="FuggobenCheck" name="fuggoben" type="checkbox"
                   {if ($termek.fuggoben)}checked="checked"{/if}>{t('Függőben')}
            <input id="AjanlottCheck" name="ajanlott" type="checkbox"
                   {if ($termek.ajanlott)}checked="checked"{/if}>{t('Ajánlott')}
            <input id="KiemeltCheck" name="kiemelt" type="checkbox"
                   {if ($termek.kiemelt)}checked="checked"{/if}>{t('Kiemelt')}
            <input id="HozzaszolasCheck" name="hozzaszolas" type="checkbox"
                   {if ($termek.hozzaszolas)}checked="checked"{/if}>{t('Hozzá lehet szólni')}
            <input id="TermekExportbanSzerepel" name="termekexportbanszerepel" type="checkbox"
                   {if ($termek.termekexportbanszerepel)}checked="checked"{/if}>{t('Termékexportokban szerepel')}
            <table>
                <tbody>
                <tr>
            ...
            <table>
                ...
                <tr>
                    <td colspan="2"><input id="OsszehajthatoEdit" type="checkbox"
                                           name="osszehajthato"{if ($termek.osszehajthato)} checked="checked"{/if}>{t('Összehajtható')}
                    </td>
                </tr>
                <tr>

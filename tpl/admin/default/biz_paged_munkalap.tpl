...
{block "itemrows"}
    <tr class="tetelsor">
        ...
        <td colspan="8" width="{$w.nevsor}"
            class="bold">{$tetel.cikkszam} {$tetel.termeknev} {foreach $tetel.valtozatok as $valtozat}{$valtozat.ertek}&nbsp;{/foreach}{if ($tetel.termekegyediazonosito|default)}({$tetel.termekegyediazonosito}) {/if}{if ($tetel.vtszszam|default)}({$tetel.vtszszam}){/if}</td>
    </tr>
    <tr class="tetelsor">

...
{block "body"}
    <div class="teto">
        ...
        {include "biz_headboxki_reverse.tpl"}
        <div class="row pull-left row-inner">
            <p class="head2label pull-left">Eredeti biz.szám: {$egyed.erbizonylatszam|default:"&nbsp;"}</p>
        ...
        <table class="teteltable pull-left">
            ...
            {foreach $egyed.tetellista as $tetel}
                <tr class="tetelsor">
                    ...
                    <td>{$tetel.termeknev} {foreach $tetel.valtozatok as $valtozat}{$valtozat.nev}: {$valtozat.ertek}&nbsp;{/foreach}{if ($tetel.termekegyediazonosito|default)}({$tetel.termekegyediazonosito}) {/if}{if ($tetel.vtszszam|default)}({$tetel.vtszszam}){/if}</td>

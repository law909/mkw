...
<form id="fakarb-form" method="post" action="/admin/termekfa/save" data-id="{$fa.id}">
	<div id="fakarb-tabs">
		...
		<div id="WebTab" class="mattkarb-page"{if ($setup.editstyle=='dropdown')} data-visible="hidden"{/if}>
			<input id="InaktivCheck" name="inaktiv" type="checkbox"{if ($fa.inaktiv)}checked="checked"{/if}>{at('Inaktív')}</input>
			<input id="Menu1LathatoCheck" name="menu1lathato" type="checkbox"{if ($fa.menu1lathato)}checked="checked"{/if}>{at('Főmenü')}</input>
			<input id="Menu2LathatoCheck" name="menu2lathato" type="checkbox"{if ($fa.menu2lathato)}checked="checked"{/if}>{at('Főmenü lenyíló')}</input>
			<input id="Menu3LathatoCheck" name="menu3lathato" type="checkbox"{if ($fa.menu3lathato)}checked="checked"{/if}>{at('Top kategória')}</input>
			<input id="Menu4LathatoCheck" name="menu4lathato" type="checkbox"{if ($fa.menu4lathato)}checked="checked"{/if}>{at('Kategória lista')}</input>
			<table><tbody>
			<tr>

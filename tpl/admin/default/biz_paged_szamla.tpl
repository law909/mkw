...
{block "itemrows"}
    <tr class="tetelsor">
        ...
        <td colspan="{if ($egyed.kedvezmenycount > 0)}10{else}8{/if}" width="{$w.nevsor}"
            class="bold">{$tetel.cikkszam} {$tetel.termeknev} {foreach $tetel.valtozatok as $valtozat}{$valtozat.ertek}&nbsp;{/foreach}{if ($setup.theme === 'superzoneb2b')}({$tetel.vtszszam}){/if}</td>
    </tr>
    <tr class="tetelsor">
